...
(function (root, factory) {
    ...
    function snHash(tagName, className, data, key) {
        // console.log("......",data,key)
        var result = document.createElement(tagName);
        result.setAttribute('h', key);
        result.className = className;
        result.appendChild(document.createTextNode("" + data));
    ...
    function scnHash(tagName, className, child, key) {
        var result = document.createElement(tagName),
            i, len;
        result.setAttribute('h', key);
        result.className = className;
    ...
    var toString = Object.prototype.toString,
        ...
        STRING_CLASS_NAME = p("type-string") + ' canCopyText',
    ...
    function _format(data, options, parentKey) {
        ...
        if (type === BOOL) {
            ...
            if (data === "") {
                result = sn("span", STRING_EMPTY_CLASS_NAME, "(无数据)");
            } else {
            ...
            //Modified BY WEN 2017.04.11
            if (options.hasOwnProperty('counter') == false) {
                options.counter = 1;
            } else {
                options.counter += 1;
            }
            //Modified BY WEN 2017.04.11 DONE
            for (key in data) {
                isEmpty = false;
                ...
                keyNode = snHash("th", OBJ_KEY_CLASS_NAME, searchHighKeys(key).name, searchHighKeys(key).key);

                if (searchHighKeys(key).flag || allHashKeyName.hasOwnProperty(key)) {
                    keyNode.className += ' highlight'
                }
                let oDelete = document.createElement('span')
                oDelete.className = 'delete-key'

                // oDelete.dataset.key = key
                oDelete.dataset.key = Field2Type[key] || key;
                oDelete.dataset.parent = parentKey

                keyNode.appendChild(oDelete)

                if (hyperlinksEnabled &&
                    typeof(value) === 'string' &&
                    indexOf.call(hyperlinkKeys, key) >= 0) {

                    valNode = scn("td", OBJ_VAL_CLASS_NAME + '-' + options.counter, linkNode(valNode, value, aTarget));
                } else {
                    valNode = scn("td", OBJ_VAL_CLASS_NAME + '-' + options.counter, valNode);
                }
            ...
            if (isSpecial) {
                ...
                // console.log("table:jh-type-object-",options.counter)
                result = scn("table", OBJECT_CLASS_NAME + "-" + options.counter, scn("tbody", '', childs));
            }
            ...
            if (data.length > 0) {
                ...
                for (key = 0, len = data.length; key < len; key += 1) {
                    ...
                    oDelete.className = 'delete-key'
                    // oDelete.dataset.key = key
                    oDelete.dataset.key = allHashKeyName[parentKey] || parentKey;
                    if (allHashKeyName.hasOwnProperty(parentKey)) {
                        keyNode.className += ' highlight';
                        keyNode.setAttribute('h', allHashKeyName[parentKey]);
                    }
                    oDelete.dataset.parent = parentKey
                    ...
                    if (hyperlinksEnabled && typeof(value) === "string") {
                        valNode = _format(value, options, key);
                        valNode = scn("td", ARRAY_VAL_CLASS_NAME + '-' + options.counter,
                ...
                options.counter -= 1;
                result = scn("table", ARRAY_CLASS_NAME + "-" + options.counter, scn("tbody", '', childs));

            } else {
                result = sn("span", ARRAY_EMPTY_CLASS_NAME + "-" + options.counter, "(Empty List)");
            }
