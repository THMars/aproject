/**
 * Created by dingyang on 2017/7/14.
 */
let oTabs,
    newData,
    objectID,
...
$(function () {
    function keydonw4Next() {
        $("#eventgraph").keydown(function (e) {
            console.log(transing);
            if (e.keyCode === 32 && !transing) {   //the ' ' key
                successHandler([eventList.shift()]);
            }
        });
    }
    ...
    switch (pathname) {
        case '/':
            ...
            break;
        default:
            let info = $('#explain_info-p>.room-item').eq(0).find('a').attr('data-room')
            let id = JSON.parse(info)['meetingID'];
            setAutoRun(auto, id);
            break;
    }
...
function getData(meetingID) {
    if (meetingInfo != null && meetingID != -1) {
        console.log(1)
        setRangeData({lastID: newId, meetingID: meetingInfo.id}, meetingUrl, (result) => {
            if (result.length > 0) {
                newId = result[result.length - 1].eventID;
            }
            result.forEach((r) => {
                eventList.push(r);
            })
        ...
        console.log(2)
        setRangeData({lastID: newId, meetingID: meetingID + ''}, meetingUrl, (result) => {
            if (result.length > 0) {
                newId = result[result.length - 1].eventID;

                successHandler([result.shift()]);
            }
            result.forEach((r) => {
                eventList.push(r);
            });
        ...
        // console.log(3)
        // setRangeData({lastID: newId, beginTime: beginTime}, initUrl, (result) => {
        //     if (result.length > 0) {
        //         newId = result[result.length - 1].eventID;
        //         successHandler([result.shift()]);
        //     }
 ...
 *  异步获取信息并设置
 ************************************************************************/
function setRangeData(data, url, cb) {
    if (isQuery) return;
    isQuery = true;
    $.ajax({
        type: "POST",
...
function displayTable(operationData, tableContainer, addClass) {
                ...
                for (let row = r; row <= r2; row++) {
                    ...
                    if (cellData !== null && cellData !== '' && cellData !== undefined) {
                        ...
                        if (isClickAll[colHeader] === 1) {  //全选
                            ...
                            if (!missionRoomRightClick.selectDataMap[colHeader].includes(newCellData)) {
                                missionRoomRightClick.selectDataMap[colHeader].push(newCellData);
                            }
                            ...
                            if (missionRoomRightClick.selectDataMap.hasOwnProperty(colHeader)) {
                                let index = missionRoomRightClick.selectDataMap[colHeader].indexOf(newCellData);
                                if (index !== -1) missionRoomRightClick.selectDataMap[colHeader].splice(index, 1);
    ...
    function otherRowRenderer(instance, td, row, col, prop, value, cellProperties) {
        ...
        if (missionRoomRightClick.selectDataMap.hasOwnProperty(colHeader)) {
            if (missionRoomRightClick.selectDataMap[colHeader].includes(colVal)) {
                $(td).addClass('table-td-isClick');
            }
        }
    }
