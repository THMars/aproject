    ...
    function _format(data, options, parentKey) {
        ...
        if (type === BOOL) {
            ...
            for (key in data) {
                ...
                options.counter -= 1;
                //Modified BY WEN 2017.04.11 DONE;
                keyNode = sn("th", OBJ_KEY_CLASS_NAME, searchHighKey(key).name);

                if (searchHighKey(key).flag) {
            ...
            if (isSpecial) {
                ...
                result = scn("table", p("type-object" + options.counter), scn("tbody", '', childs));
            }
            ...
            if (data.length > 0) {
                ...
                for (key = 0, len = data.length; key < len; key += 1) {
                    ...
                    oDelete.innerHTML = 'x'
                    // oDelete.dataset.key = key
                    oDelete.dataset.key = Field2Type[parentKey] || parentKey;
                    oDelete.dataset.parent = parentKey
        ...
        result.dataset.all = JSON.stringify(data);
        return result;
    }
